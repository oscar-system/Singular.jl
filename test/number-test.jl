@testset "number.integer_constructors for $(elemT)..." for (elemT, ringT, parentRing, R) in
      [
   ...
   @testset "parent and type relations" begin
   ...
   @testset "small integer for $U..." for U in (BigInt, ZZ, R, Nemo.ZZ)
      c = R(U(3))
      @test c isa elemT
      @test b == c
   end
   ...
   @testset "big integer for $U..." for U in (BigInt, ZZ, R, Nemo.ZZ)
      c = R(U(3)^80)
      @test c isa elemT
      @test b == c
   end
