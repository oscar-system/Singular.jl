...
function groebnerwalk3(
    ...
    if grwalktype == :standard
        walk = (x, y, z) -> standard_walk3(x, y, z)
    elseif grwalktype == :generic
        walk = (x, y, z) -> generic_walk3(x, y, z)
    elseif grwalktype == :pertubed
        walk = (x, y, z) -> pertubed_walk3(x, y, z, p)
    ...
    elseif grwalktype == :fractal_start_order
        walk =
            (x, y, z) -> fractal_walk3_start_order(
                x,
                MonomialOrder(S, S[1, :], [0]),
                MonomialOrder(T, T[1, :], T[1, :]),
            )
    ...
    elseif grwalktype == :fractal_look_ahead
        walk =
            (x, y, z) -> fractal_walk3_look_ahead(
                x,
                MonomialOrder(S, S[1, :], [0]),
                MonomialOrder(T, T[1, :], T[1, :]),
            )
    elseif grwalktype == :tran
        walk = (x, y, z) -> tran_walk3(x, y, z)
    elseif grwalktype == :fractal_combined
        walk =
            (x, y, z) -> fractal_walk3_combined(
                x,
                MonomialOrder(S, S[1, :], [0]),
                MonomialOrder(T, T[1, :], T[1, :]),
            )
...
function standard_walk3(G::Singular.sideal, S::Matrix{Int64}, T::Matrix{Int64})
    println("standard_walk3 results")
    println("Crossed Cones in: ")
    standard_walk3(G, S, T, S[1, :], T[1, :])
end

function standard_walk3(
    G::Singular.sideal,
    S::Matrix{Int64},
    T::Matrix{Int64},
    ...
    while !terminate
        ...
        println(cweight)
        global b = cweight
        G = standard_step3(G, R, cweight, Rn)
        if cweight == tweight
            terminate = true
        else
            global a = @ballocated next_weight($G, $cweight, $tweight) evals =1 samples=1
...
global c = @ballocated initials($Rn, gens($G), $cw) evals =1 samples=1

    Gw = initials(Rn, gens(G), cw)
    global d= @ballocated Singular.std(
    ...
    global e=@ballocated liftGW2($G, $R, $Gw, $H, $Rn) evals =1 samples=1
    global f= @ballocated lift($G, $R, $H, $Rn) evals =1 samples=1
    #H = liftGW2(G, R, Gw, H, Rn)
    H = lift(G, R, H, Rn)
    global g = @ballocated Singular.std($H, complete_reduction = true) evals =1 samples=1
...
#just for benchmark######
#############################
function standard_walk32(G::Singular.sideal, S::Matrix{Int64}, T::Matrix{Int64})
    println("standard_walk3 results")
    println("Crossed Cones in: ")
    standard_walk32(G, S, T, S[1, :], T[1, :])
end

function standard_walk32(
    G::Singular.sideal,
    S::Matrix{Int64},
    T::Matrix{Int64},
    cweight::Vector{Int64},
...
function generic_walk3(G::Singular.sideal, S::Matrix{Int64}, T::Matrix{Int64})
    R = base_ring(G)
    Rn = change_order(G.base_ring, T)
    global a =@ballocated next_gamma($G, $[0], $S, $T) evals =1 samples=1
...
global z = length(Singular.gens(G))
    ...
    global c =@ballocated facet_initials($G,$Lm, $v) evals =1 samples=1
    ...
    global e = @ballocated lift_generic($G, $Lm, $H) evals =1 samples=1
    H, Lm = lift_generic(G, Lm, H)
    global f= @ballocated interreduce($H, $Lm) evals =1 samples=1
    G = interreduce(H, Lm)
...
savea(df, "allocsPertubedWalk",k)
    ...
    cweight = S[1, :]
    terminate = false
    println("pertubed_walk3 results")
    println("Crossed Cones in: ")

    while !terminate
        global h = @ballocated pertubed_vector($G, $T, $p) evals =1 samples=1
        tweight = pertubed_vector(G, T, p)
        G = standard_walk32(G, S, T, cweight, tweight,k)

        if inCone(G, T, tweight)
            global i = @ballocated inCone($G, $T, $tweight) evals =1 samples=1
            terminate = true
        else
            if p == 1
                R = change_order(G.base_ring, T)
                G = Singular.Ideal(R, [change_ring(x, R) for x in gens(G)])
                global j= @ballocated Singular.std($G, complete_reduction = true) evals =1 samples=1
...
#Inlcuding:
#fractal_walk3 -> standard-version
#fractal_walk32 -> checks if the starting weight is in the inner of a cone.
#fractal_walk33 -> fractal walk expecially for conversion to the lexikographic orderig.
#                 checks if the starting weight is in the inner of a cone.
###############################################################

########################################
#Counter for the steps in the fractal_walk3
########################################
counterFr = 0
...
function fractal_walk3(
    ...
    println("Crossed Cones in: ")
    Gb = fractal_recursiv3(G, S,T, PertVecs, 1)
...
function fractal_recursiv3(
    ...
    i="-"
    while !terminate
        a = @ballocated nextT($G, $w, PertVecs[$p]) evals =1 samples=1
        t = nextT(G, w, PertVecs[p])
        if (t == [0])
            if inCone(G, T,PertVecs[p])
                h = @ballocated inCone($G, $T,PertVecs[$p]) evals =1 samples=1
                return G
            else
                i = @ballocated [pertubed_vector($G, $T, $i) for i = 1:nvars(R)] evals =1 samples=1
        ...
        Rn = change_order(R, T)
         ...
         c = @ballocated initials($R, Singular.gens($G), $w) evals =1 samples=1
        Gw = initials(R, Singular.gens(G), w)
        if p == nvars(R)
             d = @ballocated Singular.std(
         ...
         e = @ballocated liftGW2($G, $R, $Gw, $H, $Rn) evals =1 samples=1
         f = @ballocated lift($G, $R, $H, $Rn) evals =1 samples=1
        #H = liftGW2(G, R, Gw, H, Rn)
        H = lift(G, R, H, Rn)
         g = @ballocated Singular.std($H, complete_reduction = true) evals =1 samples=1
...
function fractal_walk3_start_order(
    G::Singular.sideal,
    S::MonomialOrder{Matrix{Int64},Vector{Int64}},
    T::MonomialOrder{Matrix{Int64},Vector{Int64}},
)
    ...
    global sigma = S.w
    println("fractal_walk3_withStartorder results")
    println("Crossed Cones in: ")
    Gb = fractal_walk3_recursiv_startorder(G, S, T, PertVecs, 1)
    println("Cones crossed: ", deleteCounterFr())
    return Gb
end

function fractal_walk3_recursiv_startorder(
    G::Singular.sideal,
    S::MonomialOrder{Matrix{Int64},Vector{Int64}},
    T::MonomialOrder{Matrix{Int64},Vector{Int64}},
    PertVecs::Vector{Vector{Int64}},
    ...
    while !terminate
        ...
        if p == Singular.nvars(R)
            ...
            H = fractal_walk3_recursiv_startorder(
                Singular.Ideal(R, [x for x in Gw]),
                S,
                T,
                PertVecs,
        ...
        G = Singular.std(H, complete_reduction = true)
        df = DataFrame(a=[a], b = [b],z=[z], c = [c], d = [d], e = [e], f = [f], g=[g], h=[h], i=[i],j=[j],k=[k])
        savea(df,"allocsFractalWalkstartorder")
        cleardf()
...
function fractal_walk3_lex(
    G::Singular.sideal,
    S::MonomialOrder{Matrix{Int64},Vector{Int64}},
    T::MonomialOrder{Matrix{Int64},Vector{Int64}},
)
    global PertVecs = [pertubed_vector(G, T, i) for i = 1:nvars(base_ring(G))]
    println("fractal_walk3_lex results")
    println("Crossed Cones in: ")
    Gb = fractal_walk3_recursive_lex(G, S, T, PertVecs, 1)
    println("Cones crossed: ", deleteCounterFr())
    return Gb
end

function fractal_walk3_recursive_lex(
    G::Singular.sideal,
    S::MonomialOrder{Matrix{Int64},Vector{Int64}},
    T::MonomialOrder{Matrix{Int64},Vector{Int64}},
    PertVecs::Vector{Vector{Int64}},
    ...
    while !terminate
        ...
        if t == 1 && p==1
            return fractal_walk3_recursive_lex(
                G,
                S,
                T,
                PertVecs,
...
function fractal_walk3_look_ahead(
    G::Singular.sideal,
    S::MonomialOrder{Matrix{Int64},Vector{Int64}},
    T::MonomialOrder{Matrix{Int64},Vector{Int64}},
)
    println("fractal_walk3_look_ahead results")
    println("Crossed Cones in: ")
    global PertVecs = [pertubed_vector(G, T, i) for i = 1:nvars(base_ring(G))]
    Gb = fractal_walk3_look_ahead_recursiv(G, S, T, PertVecs, 1)
    println("Cones crossed: ", deleteCounterFr())
    return Gb
end

function fractal_walk3_look_ahead_recursiv(
    G::Singular.sideal,
    S::MonomialOrder{Matrix{Int64},Vector{Int64}},
    T::MonomialOrder{Matrix{Int64},Vector{Int64}},
    PertVecs::Vector{Vector{Int64}},
    ...
    while !terminate
        ...
        if (p == Singular.nvars(R) || isbinomial(Gw))
            ...
            H = fractal_walk3_look_ahead_recursiv(
                Singular.Ideal(R, Gw),
                S,
                T,
                PertVecs,
...
function fractal_walk3_combined(
    G::Singular.sideal,
    S::MonomialOrder{Matrix{Int64},Vector{Int64}},
    T::MonomialOrder{Matrix{Int64},Vector{Int64}},
)
    ...
    println("fractal_walk3_withStartorder results")
    println("Crossed Cones in: ")
    Gb = fractal_walk3_combined(G, S, T, PertVecs, 1)
    println("Cones crossed: ", deleteCounterFr())
    return Gb
end
...
function tran_walk3(G::Singular.sideal, S::Matrix{Int64}, T::Matrix{Int64})
    cweight = S[1, :]
    tweight = T[1, :]
    println("tran_walk3 results")
    println("Crossed Cones in: ")
    R = base_ring(G)
    ...
    while !terminate
        ...
        if w == tweight
            if inCone(G, T, cweight)
                global i = @ballocated inCone($G, $T, $cweight) evals =1 samples=1
                return G
            else
                if inSeveralCones(initials(base_ring(G), gens(G), w))
                    global j = @ballocated inSeveralCones(initials(base_ring($G), gens($G), $w)) evals =1 samples=1
                    global h = @ballocated representation_vector($G, $T) evals =1 samples=1
                    tweight = representation_vector(G, T)
                    continue
                end
            end
        end
        G = standard_step32(G, R, w, Rn)
        global counter = getCounter() + 1
        println(w)
        global b=w
        R = Rn
