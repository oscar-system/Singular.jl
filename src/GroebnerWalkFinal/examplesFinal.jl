...
if case == 8 || case == 99
    ...
    #cyclic 9
    R, (z0,z1,z2, z3, z4, z5, z6, z7, z8) = Singular.PolynomialRing(
        Singular.QQ,
        ["z0", "z1", "z2", "z3", "z4", "z5", "z6", "z7", "z8"],
        ordering = Singular.ordering_M(
            ordering_as_matrix([1, 1, 1, 1, 1,1,1,1,1], :lex),
        ),
    )

    I = Singular.Ideal(R, [f1, f2, f3, f4, f5,f6,f7,f8,f9])
    I = Singular.std(I, complete_reduction = true)

    @time H = groebnerwalk(
        I,
        ordering_as_matrix([1, 1, 1, 1, 1,1,1,1,1], :lex),
        ordering_as_matrix(:lex, 9),
        :pertubed,
        5,
    )
    @time J = groebnerwalk(
    ...
    S, V = Singular.PolynomialRing(
        Singular.QQ,
        ["z0", "z1", "z2", "z3", "z4", "z5", "z6", "z7", "z8"],
        ordering = Singular.ordering_M(ordering_as_matrix(:lex, 9)),
    )
    T0 = Singular.Ideal(S, [change_ring(x, S) for x in gens(J)])
    T6 = Singular.Ideal(S, [change_ring(x, S) for x in gens(H)])
